document.addEventListener('DOMContentLoaded', function () {
    ...
    detailModal.addEventListener('show.bs.modal', async function (event) {
        ...
        try {
            ...
            // Tampilan HTML baru untuk modal
            if (detailModalBody) {
    ...
    <li class="list-group-item d-flex align-items-center ps-0 py-1 gap-3">
        ...
        <span>${data.report.pref_marah || 0}</span>
                        ...
                        <div class="row g-4 mb-4">
                            <div class="col-md-6">
                                <div class="card h-100">
                                    ...
                                    <div class="card-body d-flex align-items-center justify-content-center"><div style="height:250px; width:100%;"><canvas id="detailResponseChart"></canvas></div></div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="card h-100">
                                    <div class="card-header pb-0"><h6>Preferences Chart</h6></div>
                                    <div class="card-body"><div style="height:250px; width:100%;"><canvas id="detailPreferencesChart"></canvas></div></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mb-4">
                             ...
                             <div class="card-body"><div style="height:300px;"><canvas id="detailEngagementChart"></canvas></div></div>
                        </div>
            ...
            renderEngagementChart(JSON.parse(data.report.like_details || '{ }'), JSON.parse(data.report.share_details || '{ }'));
            renderPreferencesChart([parseInt(data.report.pref_senang || 0), parseInt(data.report.pref_biasa || 0), parseInt(data.report.pref_marah || 0)]);

        } catch (error) {
            console.error(error);
            if (detailModalBody) detailModalBody.innerHTML = `<p class="text-danger text-center py-5">${error.message}</p>`;
        }
    });
        ...
        detailEngagementChart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    fill: true,
                    data: aS.map(key => likeData[key] || 0),
                    borderWidth: 2,
                    pointRadius: 2,
                    ...
                    fill: true,
                    data: aS.map(key => shareData[key] || 0),
                    borderWidth: 2,
                    pointRadius: 2,
    ...
    function renderCommentsList(comments) {
        ...
        comments.forEach(c => {
            ...
            // Menambahkan border-bottom untuk garis pemisah
            li.className = 'list-group-item d-flex justify-content-between align-items-start px-0 py-2';
            // Menghilangkan ikon dari innerHTML
            li.innerHTML = `<div><p class="fw-bold mb-0 text-dark">${htmlspecialchars(c.username)}</p><p class="mb-0 text-sm text-muted">${htmlspecialchars(c.komentar)}</p></div>`;
    ...
    downloadPdfBtn.addEventListener('click', function () {
        if (currentReportData && currentReportData.report && currentReportData.report.id) {
            ...
            // Add chart images
            if (responseChartImg) {
                const responseChartInput = document.createElement('input');
                responseChartInput.type = 'hidden';
                responseChartInput.name = 'responseChartImg';
                responseChartInput.value = responseChartImg;
                form.appendChild(responseChartInput);
            }
            if (preferencesChartImg) {
                const preferencesChartInput = document.createElement('input');
                preferencesChartInput.type = 'hidden';
                preferencesChartInput.name = 'preferencesChartImg';
                preferencesChartInput.value = preferencesChartImg;
                form.appendChild(preferencesChartInput);
            }
            if (engagementChartImg) {
                const engagementChartInput = document.createElement('input');
                engagementChartInput.type = 'hidden';
                engagementChartInput.name = 'engagementChartImg';
                engagementChartInput.value = engagementChartImg;
                form.appendChild(engagementChartInput);
            }
            ...
            alert('Tidak dapat mengunduh PDF, data laporan tidak ditemukan. Coba buka kembali detail laporan.');
        }
    });
    ...
    if (AppConfig.isAdminView) {
        ...
        if (manageAccessModalEl) {
            ...
            // Event listener untuk setiap tombol 'Akses'
            document.querySelectorAll('.manage-access-btn').forEach(button => {
                button.addEventListener('click', async function () {
                    // Hapus log lama untuk kejelasan
                    console.clear();
                    console.log("Tombol 'Akses' diklik. Memulai proses...");

                    const fileId = this.dataset.fileId;
                    modalFileIdInput.value = fileId;
                    console.log(`File ID yang akan diproses: ${fileId}`);
                    ...
                    console.log("Semua checkbox telah di-reset (dikosongkan).");

                    try {
                        console.log("Mengirim request ke server untuk mendapatkan data akses...");
                        ...
                        if (data.status === 'success' && Array.isArray(data.client_ids)) {
                            ...
                            console.error("Format data dari server tidak sesuai. 'client_ids' bukan array atau status bukan 'success'.", data);
                        }

                    } catch (error) {
                        console.error('Terjadi kesalahan saat mengambil data akses klien:', error);
                        showNotification('Gagal mengambil data akses klien. Lihat konsol untuk detail.', true);
                    }
                });
            });

            // Event listener untuk tombol 'Save' di modal 'Manage Access'
            ...
            if (saveAccessBtn) {
                saveAccessBtn.addEventListener('click', async function () {
                    ...
                    // PERBAIKAN: Menambahkan token CSRF terbaru secara dinamis dari AppConfig
                    ...
                    try {
                        // PERBAIKAN KRITIS: URL diperbaiki (menghilangkan spasi)
                        ...
                        // Gunakan notifikasi dari utils.js jika ada, atau alert sebagai fallback
                        ...
                        // Selalu refresh token setelah request selesai untuk request berikutnya
                        refreshCsrfToken();
                    }
                });
            }
        ...
        // --- Logic untuk Delete Report via SweetAlert ---
        document.querySelector('.list-group')?.addEventListener('click', function (e) {
            const deleteButton = e.target.closest('a.text-danger');
        ...
        fetch(`${AppConfig.baseUrl}auth/get_csrf_token`)
            ...
            .then(d => {
                if (d.csrf_token_name && d.csrf_hash) {
                    ...
                    // Opsi: Update juga input hidden jika masih ada di form lain
