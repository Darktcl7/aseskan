document.addEventListener('DOMContentLoaded', function () {
    ...
    // --- Notification Helper (Bootstrap Toast - sama seperti user) ---
    ...
    // ============================================
    // CONTINUING PROJECT HANDLERS
    // ============================================

    const startContinuingBtn = document.getElementById('startContinuingBtn');
    const cancelContinuingBtn = document.getElementById('cancelContinuingBtn');

    if (startContinuingBtn) {
        startContinuingBtn.addEventListener('click', async function () {
            console.log('Ready to Start clicked!');

            if (AppConfig.continuingProject && AppConfig.continuingProject.data) {
                const projectData = AppConfig.continuingProject.data;
                const previewName = AppConfig.continuingProject.name;

                try {
                    // 1. JANGAN Load existing data
                    // loadExistingProjectData(projectData); // Hapus baris ini
                    console.log('✓ Sesi lanjutan dimulai dengan data kosong.');

                    // 2. Set preview name di UI
                    const adminPreviewNameText = document.getElementById('adminPreviewNameText');
                    if (adminPreviewNameText) {
                        adminPreviewNameText.textContent = previewName;
                    } else {
                        console.warn('Element adminPreviewNameText not found!');
                    }

                    // 3. Toggle visibility
                    const continuingProjectDisplay = document.getElementById('continuingProjectDisplay');
                    const adminPreviewNameDisplay = document.getElementById('adminPreviewNameDisplay');
                    
                    if (continuingProjectDisplay) continuingProjectDisplay.style.display = 'none';
                    if (adminPreviewNameDisplay) adminPreviewNameDisplay.style.display = 'block';
                    ...
                    // 5. Broadcast preview name ke users
                    const formData = new FormData();
                    formData.append(AppConfig.csrf.name, AppConfig.csrf.hash);
                    formData.append('preview_name', previewName);
                    ...
                    if (result.status === 'success') {
                        showNotification(`Project "${previewName}" is ready! Users can now participate.`);
                    } else {
                        throw new Error(result.message || 'Failed to broadcast preview name');
                    }

                } catch (error) {
                    console.error('Error:', error);
                    showNotification('Error setting up project: ' + error.message, true);
                }
            }
        });
    }

    if (cancelContinuingBtn) {
        cancelContinuingBtn.addEventListener('click', async function () {
            console.log('🔄 Cancel & Create New clicked');
            ...
            try {
                ...
                await fetch(AppConfig.baseUrl + 'admin/clear_continuing_project', {
                    method: 'POST',
                ...
                // Reload page to show normal form
                location.reload();
            } catch (error) {
                console.error('❌ Error clearing continuing project:', error);
                showNotification('Error clearing project. Please try again.', true);
            }
        });
    }

    // ============================================
    // FUNGSI loadExistingProjectData DIHAPUS
    // ============================================
    ...
    // ============================================
    // NOTE: DO NOT AUTO-LOAD CONTINUING PROJECT DATA HERE!
    // Data will be loaded ONLY when user clicks "Ready to Start"
    // ============================================
});